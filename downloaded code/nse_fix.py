...
def get_history(symbol, start, end, index=False, futures=False, option_type="",
                expiry_date=None, strike_price="", series='EQ', custom_parsing=False):
...
def get_history_quanta(**kwargs):
    is_custom = kwargs['custom_parsing']
    del kwargs['custom_parsing']
    url, params, schema, headers, scaling = validate_params(**kwargs)
    df = url_to_df(url=url,
                   params=params,
                   schema=schema,
                   headers=headers, scaling=scaling, is_custom=is_custom)
    return df


def url_to_df(url, params, schema, headers, scaling={}, is_custom=False):
    resp = url(**params)
    bs = BeautifulSoup(resp.text, 'lxml')

    if is_custom:
        print('custom parsing')
        csv_content = bs.find(id='csvContentDiv')
        return get_df_from_csv(csv_content.text)
...
def get_df_from_csv(df_string, sep=':'):
    if df_string:
        from io import StringIO
        import pandas as pd
        csv_str = StringIO(df_string.replace(sep, '\n'))
        return pd.read_csv(csv_str, sep=",")
